...
export function useRoundManager() {
    ...
    // ── Memoized Maps for O(1) Getters ──
    const { teamCountsMap, positionCountsMap, duplicateIdsSet, teamActiveSelectionsMap, teamUnavailableSelectionsMap, matchDetailsMap } = useMemo(() => {
        const tCounts = {}
        const pCounts = {}
        const dCounts = {}
        const tActive = {}
        ...
        if (roundData) {
            roundData.selections.forEach(s => {
                ...
                if (s.position) pCounts[tId][s.position] = (pCounts[tId][s.position] || 0) + 1
                dCounts[s.player_id] = (dCounts[s.player_id] || 0) + 1
            })

            roundData.matches?.forEach(m => {
                mDetails[m.team_id] = m
            })
        ...
        const duplicateIds = new Set(Object.entries(dCounts).filter(([, c]) => c > 1).map(([id]) => Number(id)))

        return {
            teamCountsMap: tCounts,
            positionCountsMap: pCounts,
            duplicateIdsSet: duplicateIds,
            teamActiveSelectionsMap: tActive,
            teamUnavailableSelectionsMap: tUnavail,
            matchDetailsMap: mDetails
        }
    }, [roundData])
    ...
    const getDuplicatePlayerIds = () => duplicateIdsSet
